        ...
        while index < len(self.lines) - 1:
            ...
            # step 2
                ...
                [vlan-config]
                 ...
                 #
                """
                self.cmd_append(cmdlist, "port mode 0/0 gpon", type=2)
                index, line, larr, indent = self.get_line(index)
                while not self.ishax(line):
            ...
            # step 4
                ...
                index += 1
                line = self.readline(index)
                self.cmd_append(cmdlist, line, type=1)
            ...
            # step 5
                ...
                while larr[0] == "service-port" and not self.ishax(line):
                    ...
                    while 1:
                        ...
                        index0 += 1
                    self.cmd_append(cmdlist, cmd.strip(), wtm=1.0)
                    index = index0
                    index, line, larr, indent = self.get_line(index)
            ...
            # step 6
                ...
                for i in range(0, 7):
                    index += 1
                    line = self.readline(index)
                    # if str.strip(line) == "igmp mode snooping" or str.strip(line) == "igmp version v2":
                    if str.strip(line) == "igmp mode snooping":
                        self.cmd_append(cmdlist, line, type=2)
                    # elif str.strip(line) == "igmp version v2":
                    #     self.cmd_append(cmdlist, line, type=2)
                    else:
                        self.cmd_append(cmdlist, line)
                # self.cmd_append(cmdlist, "quit\r\n")
